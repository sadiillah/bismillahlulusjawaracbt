...
import { canCreateSubjectExams } from "../../../utils/authUtils";
import { useFetchSubject } from "../../../hooks/useSubjects";
import { useCreateExam } from "../../../hooks/useExams";
import { createSubjectExamSchema, type CreateSubjectExamFormData } from "../../../schemas/examSchemas";
...
const CreateExam = () => {
  ...
  // Fetch data
  const { data: subject, isLoading: subjectLoading } = useFetchSubject(subjectId);

  // Authorization check for teachers - ensure they can only create exams for their own subjects
  useEffect(() => {
    if (subject && isTeacher && !canCreateSubjectExams(user, subject)) {
      navigate('/unauthorized');
    }
  }, [subject, isTeacher, user, navigate]);
  ...
  const {
    ...
    setValue,
  } = useForm<CreateSubjectExamFormData>({
    resolver: zodResolver(createSubjectExamSchema),
    defaultValues: {
  ...
  // Set subject_id when component mounts
  useEffect(() => {
    if (subjectId) {
  ...
  const onSubmit = async (data: CreateSubjectExamFormData) => {
    try {
          ...
          <main className="main-content flex-1 p-5">
            ...
            <div className="mt-[50px] mb-4 ml-[50px]">
              <nav className="flex items-center gap-2 text-sm">
                ...
                <ChevronRight className="w-4 h-4 text-gray-400" />
                <Link
                  to={`/dashboard/subjects/${subjectId}/exams`}
                  className="text-gray-500 hover:text-[#EF3F09] transition-colors duration-300"
                >
                  Subject Exams
                </Link>
                <ChevronRight className="w-4 h-4 text-gray-400" />
                <span className="text-brand-dark font-medium">Add Exam</span>
              </nav>
            </div>
            ...
            <div className="bg-white rounded-[20px] mb-6 pt-6 pb-6 ml-[48px] max-w-[calc(70rem-100px)]">
              <div className="flex items-center justify-between pl-2 pr-2">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h1 className="text-brand-dark text-3xl font-extrabold mb-2">Create New Exam</h1>
                    <p className="text-brand-dark text-base font-normal">Set up a new examination for your selected subject</p>
                  </div>
                </div>
                <div>
                    ...
                    <div className="flex justify-end items-center gap-4 pt-3">
                      ...
                      >
                        <span className="text-brand-white text-base font-semibold">
                          {isSubmitting || createExamMutation.isPending ? "Creating..." : "Create Exam"}
                        </span>
                      </button>
                    </div>
                  </form>
                  ...
                  <div className="bg-white rounded-[20px] px-6 py-6">
                    ...
                    {subject && (
                      <>
                        <div className="mb-4">
                          <img 
                            src={subject.photo || "https://via.placeholder.com/300x128"}
                            alt={subject.name}
                            className="w-full h-32 object-cover rounded-[12px] mb-3"
                          />
                          <h5 className="text-brand-dark text-base font-semibold">{subject.name}</h5>
                          <p className="text-gray-500 text-sm mb-2">{subject.topic?.name || "General Topic"}</p>
                        </div>
                        ...
                        {subject.teacher && (
                          <div className="border-t border-gray-200 pt-4">
                            <div className="flex items-center gap-3">
                              <img 
                                src={subject.teacher.photo || "https://via.placeholder.com/40"}
                                alt={subject.teacher.name}
                                className="w-10 h-10 rounded-full object-cover"
                              />
                              <div className="flex-1">
                                <p className="text-brand-dark text-sm font-semibold">{subject.teacher.name}</p>
