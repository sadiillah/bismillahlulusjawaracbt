...
import { School, Sparkles, Image, X, ChevronRight } from 'lucide-react';
...
import { createClassroomSchema, type CreateClassroomFormData } from '../../../schemas/classroomSchemas';
...
const AddClassroom = () => {
  ...
  const onSubmit = (data: CreateClassroomFormData) => {
    clearErrors('root');
        ...
        if (fieldErrors) {
          Object.entries(fieldErrors).forEach(([key, value]) => {
            if (Array.isArray(value) && value.length > 0) {
              setError(key as keyof CreateClassroomFormData, {
                type: 'server',
                message: value[0],
              });
            }
        ...
        <main className="flex-1 p-5">
          ...
          <div className="mt-[50px] mb-4 ml-[50px]">
            <nav className="flex items-center gap-2 text-sm">
              ...
              <ChevronRight className="w-4 h-4 text-gray-400" />
              <span className="text-brand-dark font-medium">Add Classroom</span>
            </nav>
          </div>
            ...
            <div className="flex items-center justify-between pl-2 pr-2">
              <div className="flex items-center gap-4">
                <div className="w-20 h-20 relative flex items-center justify-center rounded-[26px] overflow-hidden">
                  ...
                  <School className="w-10 h-10 text-[#1F2937] relative z-10" />
                </div>
                <div>
                  <h1 className="text-brand-dark text-3xl font-extrabold mb-2">Add New Classroom</h1>
                  <p className="text-brand-dark text-base font-normal">Create a new virtual classroom for student enrollment</p>
                </div>
              </div>
              <div>
                <button 
